...
const MiddleSection = ({ className }: { className?: string }) => {
  return (
    <section
      className={cn(
        "flex flex-col md:flex-row items-center h-full overflow-hidden",
        className
      )}
    >
      {/* text parts */}
      <section className=" flex-1 p-5 md:p-10 flex flex-col gap-2 justify-between items-start ">
        <h1 className="text-[300%] lg:text-[480%] font-bold leading-[100%]">
          Find Your Dream House
        </h1>
        <p className="">
          At Gupta Enclave, we know that finding the perfect home is more than
          just a search for a place to live—it’s about discovering a space that
          suits your lifestyle and brings comfort and joy.
          preferences.
        </p>
...
const BottomSection = ({ className }: { className?: string }) => {
  ...
  return (
    <section
      className={cn(
        "bg-background rounded-full px-5 absolute h-[6rem] bottom-[-3rem] w-[95%] md:w-[70%] shadow-md flex items-center justify-around overflow-y-auto",
        className
      )}
    >
      {options?.map((e, i) => {
        return (
          <div
            className="text-center flex-1 min-w-[5rem] flex flex-col gap-2 items-center justify-center capitalize cursor-pointer hover:scale-110 transition-all"
            key={i}
          >
            <div
              className={`w-10 flex items-center justify-center aspect-square rounded-full bg-opacity-20 ${e.color}`}
            >
              {e.icon}
            </div>
